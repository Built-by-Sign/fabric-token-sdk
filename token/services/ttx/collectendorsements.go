...
func (c *CollectEndorsementsView) requestSignatures(signers []view.Identity, verifierGetter verifierGetterFunc, context view.Context, externalWallets map[string]ExternalWalletSigner) (map[string][]byte, error) {
	...
	for i, signerIdentity := range signers {
		...
		ownerWalletStart := time.Now()
		w, werr := wm.OwnerWallet(context.Context(), signerIdentity)
		c.recordPhase(context.Context(), "sig_owner_wallet", ownerWalletStart)
		if werr == nil {
		...
		if sigSvc.IsMe(context.Context(), signerIdentity) {
			getSignerStart := time.Now()
			signer, gerr := sigSvc.GetSigner(context.Context(), signerIdentity)
			c.recordPhase(context.Context(), "sig_get_signer", getSignerStart)
			if gerr == nil {
			...
			logger.DebugfContext(context.Context(), "failed to find a signer for party [%s]: [%s]", signerIdentity, gerr)
		}
